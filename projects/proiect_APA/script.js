...
class Codec {
	...
	encode(data) {
		...
		if (mp.size === 0) {
			...
			let output_message = "Comprimarea s-a finisat si fisierul a fost trimis spre descarcare " + '\n' + "Rata compresiei : " + (data.length / final_string.length).toPrecision(6);
	...
	decode(data) {
		...
		if (k == data.length){
			alert("Fisier invalid!\nIncarcati un fisier valid .txt si incercati din nou!");
			location.reload();
			return;
		}
		if (temp === "zer") {
			let decoded_data = "";
			let output_message = "De-comprimarea s-a finisat si fisierul a fost trimis spre descarcare.";
...
window.onload = function () {
	...
	submitBtn.onclick = function () {
		...
		if (uploadedFile === undefined) {
			alert("Nici un fisier nu a fost incarcat.\nIncarcati un fisier valid .txt si incercati din nou!");
			return;
		}
		let nameSplit = uploadedFile.name.split('.');
		var extension = nameSplit[nameSplit.length - 1].toLowerCase();
		if (extension != "txt") {
			alert("Fisier invalid (." + extension + ") \nIncarcati un fisier valid .txt si incercati din nou!");
			return;
		}
		isSubmitted = true;		
		onclickChanges("Fisierul a fost incarcat cu succes", step1);
	}
	...
	encodeBtn.onclick = function () {
		...
		if (isSubmitted === false) {
			alert("Fisierul nu a fost trimis.\nApasati pe butonul 'Trimite' de la pasul precedent!!");
			return;
		}
		console.log(uploadedFile.size);
		if(uploadedFile.size === 0){
			alert("Atentie: Ati incarcat un fisier gol.\nFisierul comprimat poate fi mai mare decat cel original.\nPentru a obtine o comprimare mai eficienta, se recomanda sa incarcati fisiere mai mari.");
		}	
		else if(uploadedFile.size <= 350){
			alert("Atentie: Fisierul incarcat este foarte mic (" + uploadedFile.size +" bytes) !\nFisierul comprimat poate fi mai mare decat cel original.\nPentru a obtine o comprimare mai eficienta, se recomanda sa incarcati fisiere mai mari.");
		}
		else if(uploadedFile.size < 1000){
			alert("Atentie: Fisierul incarcat este mic (" + uploadedFile.size +" bytes) !\nFisierul comprimat poate fi mai mare decat cel original.\nPentru a obtine o comprimare mai eficienta, se recomanda sa incarcati fisiere mai mari.");
		}	
		onclickChanges("Fisierul a fost comprimat cu succes", step2);
		onclickChanges2("Comprimarea fisierului ...", "Comprimat");
		var fileReader = new FileReader();
		fileReader.onload = function (fileLoadedEvent) {
			let text = fileLoadedEvent.target.result;
			let [encodedString, outputMsg] = codecObj.encode(text);
			myDownloadFile(uploadedFile.name.split('.')[0] + "_comprimat.txt", encodedString);
	...
	decodeBtn.onclick = function () {
		...
		onclickChanges("Fisierul a fost de-comprimat cu succes", step2);
		onclickChanges2("De-comprimarea fisierului ...", "De-comprimat");
		var fileReader = new FileReader();
		fileReader.onload = function (fileLoadedEvent) {
			let text = fileLoadedEvent.target.result;
			let [decodedString, outputMsg] = codecObj.decode(text);
			myDownloadFile(uploadedFile.name.split('.')[0] + "_de-comprimat.txt", decodedString);
			ondownloadChanges(outputMsg);
		}
		fileReader.readAsText(uploadedFile, "UTF-8");
	}
...
function onclickChanges2(secMsg, word) {
	...
	msg3.innerHTML = " , " + word + " fisierul va fi descarcat automat!";
}
